...
class CommJointViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    // MARK: Properties
    ...
    func initButton() {
        ...
        cancelItem = UIBarButtonItem(barButtonSystemItem: .stop, target: self, action: #selector(CommJointViewController.searchComm(_:)))
    }
    ...
    func getData() {
        let billUrl = URL(string: "http://lowcost-env.5eg7x3kqgk.us-west-1.elasticbeanstalk.com/myweb.php?type=commJoint")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cellIdentifier = "CommJointTableViewCell"
        let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as! CommJointTableViewCell
        // Configure the cell...
        var comm : Committee?
        if self.isSearched {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "CommJointShowDetail" {
            let commDetailVC = segue.destination as! CommDetailViewController
            if let selectedCommCell = sender as? CommJointTableViewCell {
